<x-layouts.app title="Group Part">
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <h3 class="text-[16px] font-bold text-slate-800 text-center mb-1">Hapus Group</h3>
            <p class="text-[13px] text-slate-400 text-center mb-1">Apakah kamu yakin ingin menghapus</p>
            <p id="modal-group-name" class="text-[14px] font-bold text-slate-700 text-center mb-4">—</p>
    ...
    {{-- Filter Bar --}}
    <form method="GET" action="{{ route('part-groups.index') }}" class="flex flex-wrap gap-2.5 mb-4 items-center">
        @if (request('sort'))
        ...
        <div class="relative flex-1 min-w-[200px] max-w-[280px]">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama group..."
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($groups as $group)
                        <tr class="hover:bg-brand-50/40 transition-colors"
                            data-group-name="{{ addslashes($group->name) }}"
                            data-edit-url="{{ route('part-groups.edit', $group) }}"
                            data-delete-url="{{ route('part-groups.destroy', $group) }}"
                            data-parts-count="{{ $group->parts_count }}">

                            <td class="px-4 py-3">
                                <span class="text-[13px] font-semibold text-slate-800">{{ $group->name }}</span>
                            </td>

                            <td class="px-4 py-3">
                                <span
                                    class="inline-flex items-center text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-brand-50 text-brand-600">
                                    {{ $group->parts_count }} part
                                </span>
                            </td>
    ...
    @push('scripts')
        <script>
            ...
            function toggleActionMenu(event, menuId) {
                ...
                const partsCount = parseInt(row.dataset.partsCount);
            ...
            function openDeleteModal(url, name, partsCount) {
                if (partsCount > 0) {
                    alert(`Group "${name}" tidak bisa dihapus karena masih digunakan oleh ${partsCount} part.`);
                    closeAllMenus();
                    return;
                }
