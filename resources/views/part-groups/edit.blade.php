<x-layouts.app title="Edit Group Part">
    <div class="max-w-xl">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-[20px] font-extrabold text-slate-800 tracking-tight">Edit Group</h1>
                <p class="text-[13px] text-slate-400 mt-0.5">{{ $partGroup->name }}</p>
            </div>
        </div>
        <div class="bg-white border border-slate-200 rounded-2xl p-6">
            <form method="POST" action="{{ route('part-groups.update', $partGroup) }}" class="space-y-5">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-[12px] font-semibold text-slate-600 mb-1.5">Nama Group</label>
                    <input type="text" name="name" value="{{ old('name', $partGroup->name) }}"
                        placeholder="Contoh: Brakes"
