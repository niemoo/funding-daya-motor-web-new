<x-layouts.app title="Edit Toko Umum">
    <div class="max-w-xl">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-[20px] font-extrabold text-slate-800 tracking-tight">Edit Toko Umum</h1>
                <p class="text-[13px] text-slate-400 mt-0.5">{{ $generalStore->name }}</p>
            </div>
        </div>
        <div class="bg-white border border-slate-200 rounded-2xl p-6">
            <form method="POST" action="{{ route('general-stores.update', $generalStore) }}" class="space-y-5">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-[12px] font-semibold text-slate-600 mb-1.5">Nama Toko</label>
                    <input type="text" name="name" value="{{ old('name', $generalStore->name) }}"
                        placeholder="Contoh: AHASS AMARTA"
