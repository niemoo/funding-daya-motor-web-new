<x-layouts.app title="Cabang">
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <h3 class="text-[16px] font-bold text-slate-800 text-center mb-1">Hapus Cabang</h3>
            <p class="text-[13px] text-slate-400 text-center mb-1">Apakah kamu yakin ingin menghapus</p>
            <p id="modal-branch-name" class="text-[14px] font-bold text-slate-700 text-center mb-4">—</p>
    ...
    <div class="flex items-start justify-between mb-5 gap-4">
        <div>
            <h1 class="text-[20px] font-extrabold text-slate-800 tracking-tight">Master Cabang</h1>
            <p class="text-[13px] text-slate-400 mt-1">Total {{ $branches->total() }} cabang terdaftar</p>
        </div>
        @can('branches.create')
    ...
    {{-- Filter Bar --}}
    <form method="GET" action="{{ route('branches.index') }}" class="flex flex-wrap gap-2.5 mb-4 items-center">
        @if (request('sort'))
    ...
    <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                    <tr>
                        <x-sort-th column="kode_cabang" label="Kode Cabang" :currentSort="$sort" :currentDir="$dir" />
                        <x-sort-th column="nama_cabang" label="Nama Cabang" :currentSort="$sort" :currentDir="$dir" />
                        <x-sort-th column="stock_locators_count" label="Jumlah Stock" :currentSort="$sort"
                            :currentDir="$dir" />
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($branches as $branch)
                        <tr class="hover:bg-brand-50/40 transition-colors"
                            data-branch-name="{{ addslashes($branch->kode_cabang . ' — ' . $branch->nama_cabang) }}"
                            data-edit-url="{{ route('branches.edit', $branch) }}"
                            data-delete-url="{{ route('branches.destroy', $branch) }}"
                            data-stock-count="{{ $branch->stock_locators_count }}"
                            data-can-edit="{{ auth()->user()->can('branches.edit') ? 'true' : 'false' }}"
                            data-can-delete="{{ auth()->user()->can('branches.delete') ? 'true' : 'false' }}">

                            <td class="px-4 py-3">
                                <span
                                    class="text-[13px] font-semibold text-slate-800 font-mono">{{ $branch->kode_cabang }}</span>
                            </td>
                            <td class="px-4 py-3 text-[13px] text-slate-600">{{ $branch->nama_cabang }}</td>
                            <td class="px-4 py-3">
                                <span
                                    class="inline-flex items-center text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-brand-50 text-brand-600">
                                    {{ number_format($branch->stock_locators_count) }} item
                                </span>
                            </td>
                            <td class="px-4 py-3 text-[13px] text-slate-400">{{ $branch->created_at->format('d M Y') }}
                            </td>

                            @if (auth()->user()->can('branches.edit') || auth()->user()->can('branches.delete'))
                                <td class="px-4 py-3 text-center">
                                    <button onclick="toggleActionMenu(event, 'menu-{{ $branch->id }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-12 text-center">
                                ...
                                <div class="text-[14px] font-semibold text-slate-500">Tidak ada cabang ditemukan</div>
                                <div class="text-[12px] text-slate-400 mt-1">Tambah cabang baru atau import via Stock
                                    Locator</div>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if ($branches->hasPages())
            <div class="px-5 py-3.5 border-t border-slate-100 flex items-center justify-between gap-4 flex-wrap">
                <div class="text-[12px] text-slate-400">
                    Menampilkan {{ $branches->firstItem() }}–{{ $branches->lastItem() }} dari
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    @foreach ($branches->getUrlRange(1, $branches->lastPage()) as $page => $url)
                        ...
                        @elseif(abs($page - $branches->currentPage()) <= 2)
                            <a href="{{ $url }}"
    ...
    @push('scripts')
        <script>
            ...
            function toggleActionMenu(event, menuId) {
                ...
                const stockCount = parseInt(row.dataset.stockCount);
                ...
                if (canDelete) {
                    ...
                    menuItems += `
                    <button onclick="openDeleteModal('${deleteUrl}', decodeURIComponent('${encodeURIComponent(branchName)}'), ${stockCount})"
            ...
            function openDeleteModal(url, name, stockCount) {
                if (stockCount > 0) {
                    alert(
                        `Cabang "${name}" tidak bisa dihapus karena masih memiliki ${stockCount.toLocaleString()} data stock.`);
                    closeAllMenus();
                    return;
                }
                document.getElementById('modal-branch-name').textContent = name;
